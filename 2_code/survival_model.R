...
# Model for publication bias includes all moderators and random effects, as well as year to account for time lag and precision (wi) **** D.N.: Switch to model formula. You do not want to include both wi and vi in the same model. Just use vi. Also, simplify the model so the intercept is marginalised and is more meaningful. 
pub.model_z <-rma.mv(yi ~ waterpotdiff_scaled + T_scaled + waterpotdiff_scaled:T_scaled + pub_year + vi, V = Vmat, random = list(~1|paper_no, ~1|animal, ~1|row_count), R = list(animal=R_phylo), mod = , data = Sur_dataOR, method = 'ML')
summary(pub.model_z)
